...
// Tool Labs
// チャタリング防止のための待ち時間(ms)
#define TIME_FOR_SB 20

// Tool Labs
// シフトキーのポート定義
#define SH_PORT  PORTAbits.RA3
...
typedef struct __attribute__((packed))
{
    ...
    {
        // Tool Labs
        // キーの修飾(command/shift/option)はbits構造体を使用せずに
        // 直接ビット指定するためunionに1バイトの"value"を追加
        uint8_t value;
...
typedef struct __attribute__((packed))
{
    ...
    // Tool Labs
    // キー割り当て設定用にOUTPUT reportバッファ(ホスト→デバイス)を8バイトに変更
    uint8_t value[8];
...
// Tool Labs
// キーコード配列の宣言(外部変数)
extern unsigned char keyCode[NumberOfSwitches];
extern unsigned char keyModifier[NumberOfSwitches];
extern unsigned char shiftedKeyCode[NumberOfSwitches];
extern unsigned char shiftedKeyModifier[NumberOfSwitches];
...
void APP_KeyboardTasks(void)
{
    ...
    if(HIDTxHandleBusy(keyboard.lastINTransmission) == false)
    {
        ...
        // Tool Labs
        // ここから各ポートのスイッチ押下状態を確認して、
        // 押されていたらその割り当てキーの値をINPUT reportに入れてホストに渡す
        // この関数を作って12回呼び出すように後で変更する
        if(BUTTON_IsPressed(BUTTON_S1) == true) {
            if(keyboard.waitingForRelease == false) {
                ...
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S1];
                    inputReport.modifiers.value = keyModifier[BUTTON_S1];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S1];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S1];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
            keyboard.waitingForRelease = false;
        }

        if(BUTTON_IsPressed(BUTTON_S2) == true) {
            if(keyboard.waitingForRelease == false) {
                keyboard.waitingForRelease = true;
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S2];
                    inputReport.modifiers.value = keyModifier[BUTTON_S2];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S2];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S2];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
        ...
        if(BUTTON_IsPressed(BUTTON_S3) == true) {
            if(keyboard.waitingForRelease == false) {
                ...
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S3];
                    inputReport.modifiers.value = keyModifier[BUTTON_S3];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S3];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S3];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
        ...
        if(BUTTON_IsPressed(BUTTON_S4) == true) {
            if(keyboard.waitingForRelease == false) {
                ...
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S4];
                    inputReport.modifiers.value = keyModifier[BUTTON_S4];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S4];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S4];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
        ...
        if(BUTTON_IsPressed(BUTTON_S5) == true) {
            if(keyboard.waitingForRelease == false) {
                ...
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S5];
                    inputReport.modifiers.value = keyModifier[BUTTON_S5];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S5];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S5];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
        ...
        if(BUTTON_IsPressed(BUTTON_S6) == true) {
            if(keyboard.waitingForRelease == false) {
                ...
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S6];
                    inputReport.modifiers.value = keyModifier[BUTTON_S6];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S6];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S6];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
        ...
        if(BUTTON_IsPressed(BUTTON_S7) == true) {
            if(keyboard.waitingForRelease == false) {
                keyboard.waitingForRelease = true;
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S7];
                    inputReport.modifiers.value = keyModifier[BUTTON_S7];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S7];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S7];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
        ...
        if(BUTTON_IsPressed(BUTTON_S8) == true) {
            if(keyboard.waitingForRelease == false) {
                keyboard.waitingForRelease = true;
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S8];
                    inputReport.modifiers.value = keyModifier[BUTTON_S8];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S8];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S8];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
        ...
        if(BUTTON_IsPressed(BUTTON_S9) == true) {
            if(keyboard.waitingForRelease == false) {
                ...
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S9];
                    inputReport.modifiers.value = keyModifier[BUTTON_S9];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S9];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S9];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
        ...
        if(BUTTON_IsPressed(BUTTON_S10) == true) {
            if(keyboard.waitingForRelease == false) {
                keyboard.waitingForRelease = true;
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S10];
                    inputReport.modifiers.value = keyModifier[BUTTON_S10];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S10];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S10];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
        ...
        if(BUTTON_IsPressed(BUTTON_S11) == true) {
            if(keyboard.waitingForRelease == false) {
                keyboard.waitingForRelease = true;
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S11];
                    inputReport.modifiers.value = keyModifier[BUTTON_S11];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S11];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S11];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
        ...
        if(BUTTON_IsPressed(BUTTON_S12) == true) {
            if(keyboard.waitingForRelease == false) {
                keyboard.waitingForRelease = true;
                /* Set the only important data, the key press data. */
                if( SH_PORT ) {
                    inputReport.keys[0]   = keyCode[BUTTON_S12];
                    inputReport.modifiers.value = keyModifier[BUTTON_S12];
                } else {
                    inputReport.keys[0]   = shiftedKeyCode[BUTTON_S12];
                    inputReport.modifiers.value = shiftedKeyModifier[BUTTON_S12];
                }
                __delay_ms(TIME_FOR_SB);
            }
        } else {
...
static void APP_KeyboardProcessOutputReport(void)
{
    ...
    // Tool Labs
    // ホストからのLED制御のサンプルコードは使わないのでコメントアウト
#if 0
    ...
    // Tool Labs
    // キーデータ書き換え
    // outputReport.value[0]が0x01だったらキーコード書き換えパケット
    if( outputReport.value[0] == 0x01 ){
        // 設定キー番号取得
        keyNumber = (outputReport.value[1] & 0x7f) - 1;
        
        // シフトキーOff/On判定
        if( (outputReport.value[1] & 0x80) == 0x00 ){
            // シフトキーOFFの場合
            // 現在の設定を変更
            keyCode[keyNumber]     = outputReport.value[4];
            keyModifier[keyNumber] = outputReport.value[5];
            // EEPROMデータを書き換え
            eeprom_write(keyNumber * 2, outputReport.value[4]);
            eeprom_write(keyNumber*2 + 1, outputReport.value[5]);
        } else {
            // シフトキーONの場合
            // 現在の設定を変更
            shiftedKeyCode[keyNumber]     = outputReport.value[4];
            shiftedKeyModifier[keyNumber] = outputReport.value[5];
            // EEPROMデータを書き換え
            eeprom_write(keyNumber*2 + NumberOfSwitches,     outputReport.value[4]);
            eeprom_write(keyNumber*2 + NumberOfSwitches + 1, outputReport.value[5]);            
        }
    }
}
